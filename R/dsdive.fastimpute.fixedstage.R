...
#' @export
#'
dsdive.fastimpute.fixedstage = function(M, depth.bins, depths, times, s0, beta, 
  ...
  for(i in 1:(nt-1)) {
    ...
    # extract initial and final state indices
    s.range = findInterval(c(t0, tf), t.stages) + 1
    
    # impute trajectory segment
    br = dsdive.fastbridge.fixedstage(M = M, depth.bins = depth.bins, 
                                      d0 = depths[i], d0.last = d0.last, 
                                      df = depths[i+1], beta = beta, 
                                      lambda = lambda, sub.tx = sub.tx, 
                                      surf.tx = surf.tx, t0 = t0, 
                                      tf = tf, s0 = s.range[1], sf = s.range[2],
                                      t.stages = t.stages,
                                      inflation.factor.lambda = 
                                        inflation.factor.lambda,
                                      verbose = verbose, 
                                      precompute.bridges = precompute.bridges,
                                      lambda.max = lambda.max, model = model, 
                                      t0.dive = t0.dive)
