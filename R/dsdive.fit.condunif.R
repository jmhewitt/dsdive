...
dsdive.fit.condunif = function(
  ...
  impute = function(depth.bins, depths, times, beta, lambda, t.stages) {
    # initialize with truncpois
    dsdive.impute_segments(
      depth.bins = depth.bins, depths = depths, times = times, beta = beta, 
      lambda = lambda, s0 = 1, verbose = FALSE, method.N = 'truncpois',
      inflation.factor.lambda = inflation.factor.lambda, N.max = 100, 
      t.sbreaks = t.stages)
  }
  
  impute.gibbs = function(depth.bins, depths, times, beta, lambda, 
                          imputed.cond) {
    # update with conditional uniformization
    dsdive.impute_segments_cond(
      depth.bins = depth.bins, depths = depths, times = times, beta = beta, 
      lambda = lambda, inflation.factor.lambda = inflation.factor.lambda, 
      verbose = FALSE, t.sbreaks = imputed.cond$t.stages, 
      t.cond = imputed.cond$times)
  }
